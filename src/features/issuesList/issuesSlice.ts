...
import { IIssueCollection, IIssueModel } from '../../communications/issue/issue.types'

interface IssuesState {
  issuesByNumber: Record<number, IIssueModel>
...
const issues = createSlice({
  ...
  reducers: {
    ...
    getIssueSuccess(state, { payload }: PayloadAction<IIssueModel>) {
      const { number } = payload
      state.issuesByNumber[number] = payload
      state.isLoading = false
      state.error = null
    },
    getIssuesSuccess(state, { payload }: PayloadAction<IIssueCollection>) {
